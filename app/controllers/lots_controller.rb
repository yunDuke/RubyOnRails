class LotsController < ApplicationController
  ...
  def index
      ...
      marker.infowindow lot.markerText
    end
  end
  ...
  def show
    @lot = Lot.find(params[:id])
      ...
      # .paginate(page: params[:page],:per_page => 5)
      @hash = Gmaps4rails.build_markers(@lot) do |lot, marker|
      marker.lat lot.latitude
      marker.lng lot.longitude
      marker.infowindow lot.markerText
    end
  ...
  private
    ...
    def lot_params
      params.require(:lot).permit(:title, :description,:image,:num,:latitude, :longitude,:rest, :markerText)
    end
end
